...
def get_spatial_temporal_demand(target_df, source_df):
    """
    For each row in target_df, finds the demand at the same geohash on the same timestamp in source_df.
    If the geohash has no record at that timestamp, finds the nearest geohash in source_df that HAS a record
    at that exact timestamp and copies its demand. This completely eliminates NaNs and preserves diurnal shape.
    """
    mapped_demands = []
    ...
    for ts in unique_timestamps:
        ...
        if len(source_ts) == 0:
            # Fallback (should not happen for valid timestamps)
            source_ts = source_df.copy()
            
        candidates = source_ts[['geohash', 'lat', 'lon', 'demand']].drop_duplicates(subset=['geohash'])
        ...
        # Build exact geohash map for O(1) matching of non-missing entries
        exact_map = dict(zip(cand_ghs, cand_demands))
        
        ts_demands = []
        for idx, row in target_ts.iterrows():
            gh = row['mapped_geohash'] if 'mapped_geohash' in row else row['geohash']
            if gh in exact_map:
                ts_demands.append(exact_map[gh])
            else:
                # Spatial distance search among candidates that have this timestamp
...
def main():
    ...
    # 2. Map missing test geohashes to nearest known training geohashes globally (for fallback/categoricals)
    ...
    if len(missing_geohashes) > 0:
        print(f"Mapping {len(missing_geohashes)} missing test geohashes to nearest train geohashes globally...")
        train_coords = []
    ...
    # 6. Map features to day 49 training set using robust spatial-temporal mapping
    print("Mapping historical demand features to train_49...")
    df_train_49 = get_spatial_temporal_demand(train_49, train_48)
    ...
    # 7. Map features to test set using robust spatial-temporal mapping (No NaNs, preserves diurnal shape!)
    print("Mapping historical demand features to test...")
    df_test = get_spatial_temporal_demand(test, train_48)
    ...
    df_test['demand_day48'] = df_test['demand_day48'].fillna(df_test['gh_mean']).fillna(global_mean)
    ...
    # We must sort test set back to the original order of test_indices
    ...
    rf_day49 = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)
