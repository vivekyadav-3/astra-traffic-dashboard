...
def main():
    ...
    le_gh = LabelEncoder()
    combined_train['geohash_enc'] = le_gh.fit_transform(combined_train['geohash'])
    mapping_gh = dict(zip(le_gh.classes_, range(len(le_gh.classes_))))
    df_train_49['geohash_enc'] = df_train_49['geohash'].map(mapping_gh).fillna(-1).astype(int)
    df_test['geohash_enc'] = df_test['mapped_geohash'].map(mapping_gh).fillna(-1).astype(int)
    ...
    feature_cols = [
        ...
        'NumberofLanes', 'Temperature', 'day', 'geohash_enc'
