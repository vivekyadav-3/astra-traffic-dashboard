...
def main():
    ...
    ensemble_preds = 0.3 * preds_et_day49 + 0.2 * preds_rf_day49 + 0.3 * preds_et_comb + 0.2 * preds_rf_comb
    
    # Pure formula shift
    pure_preds = np.clip(df_test['demand_day48'] + df_test['shift_diff'], 0.0, 1.0)
    
    # 95% Ensemble + 5% Pure Linear Regularization
    final_preds = 0.95 * ensemble_preds + 0.05 * pure_preds
