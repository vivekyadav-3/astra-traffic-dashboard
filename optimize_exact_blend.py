...
def main():
    ...
    # Grid search for blending weights of:
    # final = (1 - alpha) * (w_et * oof_et + w_rf * oof_rf + w_lgb * oof_lgb) + alpha * pred_wmean
    best_r2 = -1
    best_params = None
    
    for alpha in [0.0, 0.05, 0.1, 0.15, 0.2]:
        for w_et in np.linspace(0, 1, 11):
            for w_rf in np.linspace(0, 1 - w_et, 11):
                w_lgb = 1.0 - w_et - w_rf
                if w_lgb < -1e-5:
                    continue
                
                ml_pred = w_et * oof_et + w_rf * oof_rf + w_lgb * oof_lgb
                blend = (1 - alpha) * ml_pred + alpha * df_train_49['pred_wmean']
    ...
    print(f"  Alpha (weight on pure formula): {best_params[0]:.2f}")
    print(f"  ML Weights: ET={best_params[1]:.2f}, RF={best_params[2]:.2f}, LGB={best_params[3]:.2f}")
    print(f"  R2: {best_r2:.5f}")
