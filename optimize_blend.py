...
def main():
    ...
    print(f"ET OOF R2: {r2_score(df_train_49['demand'], oof_et):.5f}")
    print(f"RF OOF R2: {r2_score(df_train_49['demand'], oof_rf):.5f}")
    print(f"LGB OOF R2: {r2_score(df_train_49['demand'], oof_lgb):.5f}")
    
    # Let's search for the best weights using a simple grid search
    best_r2 = -1
    best_w = None
    ...
    for w_et in np.linspace(0, 1, 21):
        for w_rf in np.linspace(0, 1 - w_et, 21):
            ...
            blend = w_et * oof_et + w_rf * oof_rf + w_lgb * oof_lgb
            r2 = r2_score(df_train_49['demand'], blend)
            if r2 > best_r2:
                best_r2 = r2
                best_w = (w_et, w_rf, w_lgb)
                
    print(f"\nBest Blend: ET={best_w[0]:.2f}, RF={best_w[1]:.2f}, LGB={best_w[2]:.2f} -> R2: {best_r2:.5f}")
