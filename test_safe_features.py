...
def main():
    ...
    # Safe features
    safe_feats = [
        'lat', 'lon', 'demand_day48', 'shift_diff', 'shift_ratio', 'gh_mean',
        'NumberofLanes'
    ] + [c + '_enc' for c in cat_cols]
    
    print(f"Safe features count: {len(safe_feats)}")
    print("Safe features:", safe_feats)
    
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    
    # Evaluate ExtraTrees on safe features
    ...
    r2_safe = r2_score(df_train_49['demand'], preds_all)
    print(f"ExtraTrees(n=300) with SAFE features CV R2: {r2_safe:.5f}")
    
    # Feature importances
    et.fit(df_train_49[safe_feats], df_train_49['demand'])
    imp = pd.Series(et.feature_importances_, index=safe_feats).sort_values(ascending=False)
    print("\nFeature Importances:")
    print(imp)
